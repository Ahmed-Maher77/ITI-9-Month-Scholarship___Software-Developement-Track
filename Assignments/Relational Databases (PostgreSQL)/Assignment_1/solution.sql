...
where 
is_active = true and gender = 'Female' and enroll_date > '2021-01-01';
...
select * from courses
where 
(course_name ilike '%Systems%' OR course_name ilike '%Analysis%')


-- Q7
select * from students
where
dept_id in (1,2,3) and gpa > 3.0;


-- Q8
select d.dept_name, count(s.*) as student_count, avg(s.gpa) avg_gpa, min(s.gpa) as min_gpa, max(s.gpa) as max_gpa 
from departments d
...
-- Q10
select p.first_name || p.last_name as prof_name, 
