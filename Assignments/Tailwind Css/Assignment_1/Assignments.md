# Assignment 1: Workcation Landing Page

## Overview
This assignment demonstrates a responsive landing page for "Workcation" - a service that helps users find work-friendly rentals in beautiful locations. Built with Tailwind CSS v4.
...
```json
"build:css": "node ./node_modules/@tailwindcss/cli/dist/index.mjs -i ./input.css -o ./output.css --watch"
```
...
## Key Learning Points
1. **Mobile-first responsive design** using Tailwind's breakpoint system
2. **CSS Grid** for complex layouts (`2xl:grid-cols-5`)
3. **Flexbox** for card alignments
4. **Component-like card patterns** with consistent styling
5. **Interactive states** (hover, focus, active) for better UX
6. **Tailwind CSS v4** with new `@import "tailwindcss"` syntax
7. **CLI-based build process** with watch mode for development