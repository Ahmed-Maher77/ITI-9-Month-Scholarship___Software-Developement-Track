...
// In serverless environments (like Vercel), do not attempt to write to local files
...
if (!isServerless) {
  try {
    ...
    console.error('Failed to initialize local file logger:', err.message);
  }
}
 ...
 * Always logs to file in combined format if available, otherwise acts as no-op
 */
const fileLogger = logFileStream
  ? morgan(fileFormat, { stream: logFileStream })
  : (req, res, next) => next();
